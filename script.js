...
(function () {
    d3.csv("data/imdb_tomatoes.csv").then(data => {
        ...
        // Deduplicate titles, append (year) if necessary
        const grouped2 = d3.rollups(
            data,
            v => v.map(d => ({ ...d })),
            d => d.title.trim().toLowerCase()
        );
        ...
        const seen2 = new Set();
        const deduped = [];
        for (const [baseTitle, entries] of grouped2) {
            if (entries.length > 1) {
                for (const e of entries) {
                    const uniqueTitle = `${e.title.trim()} (${e.release_year})`;
                    if (!seen2.has(uniqueTitle)) {
                        seen2.add(uniqueTitle);
